$().ready(function () {
  ...
  function callAPI(city) {
    var apiKey = "db8b5efe9618f33710cd0556eb302ced";
    var urlQuery =
    ...
    $.ajax({
      ...
      $("#current-weather").append(
        $("<h2>")
          .text(data.name + " " + getFormatedDate())
          .addClass("card-title")
          .css("display", "inline")
      );
  ...
  function getForecast(lat, lon) {
    ...
    $.ajax({
      ...
      for (let i = 1; i < 6; i++) {
        ...
        $("<h3>")
          .addClass("card-title")
          .text(getDateEpoch(dayData.dt))
          .appendTo(forecastCardBody);
        var forecastIconURL =
